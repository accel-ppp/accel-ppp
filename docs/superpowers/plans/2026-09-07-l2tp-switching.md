...
**Follow-up measurement that changes how to read the above: a plain `iperf3` benchmark between the same two VMs, entirely outside accel-ppp**, found the underlying path's own UDP capacity is dramatically lower than its TCP capacity — 9.15 Gbit/s TCP vs. ~1.2-1.4 Gbit/s UDP, and that UDP ceiling did *not* rise with more parallel streams (`-P 4`), which points at the virtualized network path's own small-packet handling rather than either host's CPU or this feature's single relay thread. The "burst that broke it" figures above were the *local* `write()` acceptance rate into the switch's own upstream kernel socket, not a measurement of what actually left the wire toward the downstream leg — so at least part of what looked like a pure relay-design limitation is really the switch being asked to sustain a real UDP rate at or above this specific VM pair's actual ceiling, which no amount of buffer or threading changes on the switch's own side could ever exceed. `docs/l2tp_switching.md` now documents the `iperf3` TCP-vs-UDP-vs-parallel-streams-vs-larger-payload procedure, plus an `mpstat` CPU check during the single-stream run, as the first diagnostic step for any future throughput report. That last check turned out to matter: CPU stayed mostly idle on both hosts throughout the capped UDP test, and the wire-level packets-per-second rate stayed roughly constant (~100K-130K pps) whether the payload was 1400 bytes or fragmented into several IP packets from a larger one, while the achieved bitrate moved with payload size — the specific signature of a packets-per-second policer somewhere in the network path itself (a common anti-UDP-flood measure at cloud/hosting providers), not CPU/generation overhead on either host and not a raw bandwidth cap. This distinction matters for what's actually fixable: a PPS ceiling enforced outside both hosts cannot be raised by anything done to this feature.

**Follow-up fix, once the above made clear which part of the problem actually was fixable: switch-mode sockets now request 4 MiB `SO_RCVBUF`/`SO_SNDBUF` (via `SO_RCVBUFFORCE`/`SO_SNDBUFFORCE`, bypassing the usual `net.core.rmem_max`/`wmem_max` clamp — accel-ppp already needs the equivalent capability for `pppol2tp` itself), and the outbound `splice(2)` in `l2tp_switch_link_read()` now retries `ENOMEM`/`ENOBUFS` with a short bounded backoff (up to ~1s total) instead of treating them as fatal.** The receive-side kernel buffer overflow and the network's own PPS ceiling are both still real and unchanged by this — a buffer only changes how big a *momentary* burst can be absorbed before loss starts, not a sustained rate enforced outside the host — but the specific failure mode of "a burst disconnects an otherwise-healthy call" is what this targets, and it worked: re-running the same burst sizes that previously disconnected the call (200 and 2,000 back-to-back 1400-byte writes) now relay with zero loss and the call stays up, and even a 100,000-write / 140 MB burst (far beyond anything in the original characterization) no longer disconnects the call at all — it degrades to lossy delivery (confirmed via the receive-side path, roughly 40% delivered, matching the path's own PPS ceiling from the measurement above) rather than tearing the call down. Full `l2tp_switch` suite (17 tests) re-verified passing with no regressions after this change.
...
Added as a sibling `test_switch_metrics.py` rather than extending `test_switch_show.py`, for one reason found while writing it: `helpers.start_instance()`'s `[modules]` section is fixed to `log_syslog`/`l2tp` — a *second* `[modules]` section added via its `extra=` parameter is not merged with the first. Confirmed on a real VM: with two separate `[modules]` blocks in the same config file, `metrics` silently never loads (no error, no log line — the `/metrics` port simply never opens, connection refused). `test_switch_metrics.py` therefore builds its own config from scratch via `config.make_tmp()` (matching `test_switch_tunnel.py`'s existing pattern for the same reason), listing `metrics` alongside `l2tp` in one `[modules]` block from the start, and reads `/metrics` with the standard-library `http.client` (matching `tests/accel-pppd/general/test_metrics.py`'s own existing convention for hitting this endpoint, rather than shelling out to `curl`).
